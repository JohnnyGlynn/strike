...
func PreparedClientStatements(ctx context.Context, dbpool *pgxpool.Pool) (*ClientDB, error) {
	...
	if _, err := poolConnection.Conn().Prepare(ctx, statements.CreateChat, "INSERT INTO chats2 (chat_id, chat_name, initiator, participants, state) VALUES ($1, $2, $3, $4, $5)"); err != nil {
		return nil, err
	}
